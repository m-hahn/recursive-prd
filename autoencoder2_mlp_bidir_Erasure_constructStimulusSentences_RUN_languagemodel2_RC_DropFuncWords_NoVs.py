...
class Autoencoder(torch.nn.Module):
  ...
  def forward(self, numeric, train=True, printHere=True):
      ...
      numeric_noised = [[x if itos_total[x] not in ["that", "the", "who"] else 0 for x in y] for y in numeric.cpu().t()] # [".", "the", "who", "that", "was"]
...
#   with open(f"temporary-stimuli/{args.surpsFile}", "w") as outFileSurps:
    ...
    #      continue
          ...
          for RUN in range(args.NUMBER_OF_RUNS):
             ...
             V3 = "knew"
             if condition == 0:           
                resultNumeric = torch.cat([resultNumeric, torch.LongTensor([stoi_total[V2] for _ in range(args.batchSize*args.SAMPLES_PER_BATCH)]).cuda().view(-1, 1), torch.LongTensor([stoi_total[V3] for _ in range(args.batchSize*args.SAMPLES_PER_BATCH)]).cuda().view(-1, 1), torch.LongTensor([stoi_total["people"] for _ in range(args.batchSize*args.SAMPLES_PER_BATCH)]).cuda().view(-1, 1), torch.LongTensor([stoi_total["."] for _ in range(args.batchSize*args.SAMPLES_PER_BATCH)]).cuda().view(-1, 1)], dim=1)
             else:
                resultNumeric = torch.cat([resultNumeric, torch.LongTensor([stoi_total[V3] for _ in range(args.batchSize*args.SAMPLES_PER_BATCH)]).cuda().view(-1, 1), torch.LongTensor([stoi_total["people"] for _ in range(args.batchSize*args.SAMPLES_PER_BATCH)]).cuda().view(-1, 1), torch.LongTensor([stoi_total["."] for _ in range(args.batchSize*args.SAMPLES_PER_BATCH)]).cuda().view(-1, 1)], dim=1)
