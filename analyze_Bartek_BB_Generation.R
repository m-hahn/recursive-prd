...
data = data.frame()
for(model in models) {
   data2 = read.csv(paste("outputGeneration/Bartek_BB_", model, sep=""), sep="\t")
   data2$Model = model
...
summary(glmer(as.numeric(as.character(VBD_Generated))/100 ~ pp_rc * emb_c + someIntervention + (1 + pp_rc + emb_c + someIntervention|item), data=vb, family="binomial")) 
# When removing slopes:
#pp_rc            -0.37753    0.07988  -4.726 2.29e-06 *** (explains why verbs look more look more difficult after RCs)
#emb_c             0.95120    0.06606  14.400  < 2e-16 *** (explains facilitation in embedded environments)
# But in general, does not explain away locality
