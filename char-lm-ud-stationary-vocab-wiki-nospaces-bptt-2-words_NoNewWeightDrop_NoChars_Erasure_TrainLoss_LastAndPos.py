...
modules_memory = [memory_mlp_inner, memory_mlp_outer, mlp_inner_from_pos, positional_embeddings]
...
def forward(numeric, train=True, printHere=False):
  ...
  #        beginning_chars = torch.where(forRestart.unsqueeze(0).unsqueeze(2) == 1, zeroBeginning_chars, beginning_chars)
      ...
      numeric_positions = torch.LongTensor(range(args.sequence_length)).cuda()
      numeric_embedded = positional_embeddings(numeric_positions)
      print(numeric_embedded.size())
      numeric_transformed = memory_mlp_inner_from_pos(numeric_embedded)
      memory_hidden = sigmoid(memory_mlp_outer(relu(numeric_transformed + memory_mlp_inner(embedded_everything))))
...
#      global runningAveragePredictionLoss
      ...
      if printHere:
         ...
         for i in range((args.sequence_length)):
            print((losses[0][0] if i == args.sequence_length-1 else None , itos_total[numericCPU[i+1][0]], itos_total[numeric_noisedCPU[i+1][0]], memory_hidden_CPU[i+1]))
