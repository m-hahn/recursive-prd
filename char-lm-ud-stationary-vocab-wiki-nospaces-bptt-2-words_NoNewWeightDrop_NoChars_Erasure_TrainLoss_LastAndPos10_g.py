...
parser.add_argument("--learning_rate", type = float, default= random.choice([0.00001, 0.00002, 0.00005, 0.0001, 0.0002, 0.0003, 0.0004]))
...
#      print(numeric_transformed.size(), embedded_everything.size())
      ...
      memory_hidden_logit = attention_bilinear_term
 #     print("----")
...
#      quit()
      ...
      if printHere:
         ...
         for i in range((args.sequence_length)):
            print((losses[0][0] if i == args.sequence_length-1 else None , itos_total[numericCPU[i+1][0]], itos_total[numeric_noisedCPU[i+1][0]], memory_hidden_CPU[i+1], float(baselineValues[0]) if i == args.sequence_length-1 else "", float(attention_bilinear_term[i+1,0,0])))
