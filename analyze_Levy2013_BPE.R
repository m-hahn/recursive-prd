...
agreement = paste(raw.spr.data$Word, "</w>", sep="") == raw.spr.data$RegionLSTM
...
raw.spr.data = raw.spr.data %>% mutate(Local = (Condition %in% c("A", "D"))) %>% mutate(Local.C = Local-mean(Local))
