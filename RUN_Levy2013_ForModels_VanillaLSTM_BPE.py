...
models = [x for x in os.listdir(path) if x.startswith("estimates-"+language+"_") and "words_NoNewWeightDrop_BPE_RunTest.py" in x]
...
#       print >> outFile, "\t".join([str(x) for x in [ID, script, surprisal]])
       ...
       for section in ["1a"]:
          command = ["/u/nlp/anaconda/main/anaconda3/envs/py37-mhahn/bin/python", "RUN_Levy2013_"+script, "--language=russian", "--load-from="+ID, "--section="+section]
