...
for RUN in range(args.NUMBER_OF_RUNS):
   MASKs.append([[random.random() > args.deletion_rate for _ in range(args.batchSize)] for y in range(args.sequence_length)])
...
class Autoencoder(torch.nn.Module):
  ...
  def forward(self, numeric, train=True, printHere=True):
      ...
      numericCPUT = numeric.cpu().t()
      numeric_noised = [[numericCPUT[i,j] if MASK[j][i] or itos_total[numericCPUT[i,j]] == "." else 0 for j in range(args.sequence_length)] for i in range(args.batchSize)]
