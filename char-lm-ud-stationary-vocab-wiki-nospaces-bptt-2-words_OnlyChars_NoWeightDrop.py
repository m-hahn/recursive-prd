...
bernoulli_input = torch.distributions.bernoulli.Bernoulli(torch.tensor([1-args.weight_dropout_in for _ in range(args.batchSize *  args.word_embedding_size)]).cuda())
...
#      print(embedded_chars[:,5,:].mean(dim=1)[numeric[:-1,5] == 3])
      ...
      if train:
         ...
         mask = mask.view(1, args.batchSize, args.word_embedding_size)
