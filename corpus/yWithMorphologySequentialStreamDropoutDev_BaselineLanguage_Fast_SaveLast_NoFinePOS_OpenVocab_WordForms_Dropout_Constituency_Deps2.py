# yWithMorphologySequentialStreamDropoutDev_BaselineLanguage_Fast_SaveLast_NoFinePOS_OpenVocab_WordForms_Dropout_Constituency_Deps2.py
# For SUBJECT-extracted relatives, find how often [VB NP] vs [VB NP ...] occur as subject relatives, in subject position.
   ...
   if type(children[0]) != nltk.tree.Tree:
      ...
      if label == "SBAR":
        if len(childrenAsTrees) > 1:
           if len(childrenAsTrees) == 2 and childrenAsTrees[0]["category"] in ["IN", "WHNP"] and childrenAsTrees[1]["category"] == "S" and childrenAsTrees[1]["dependency"] == "acl:relcl": # Relative clause
             if childrenAsTrees[0]["dependency"] == "nsubj": # SUBJECT Relatives
               if sentence[childHeads[1]-1]["dep"] in ["nsubj", "obj"]:
          #     _ = 0
...
#                      assert len(childrenAsTrees[1]["children"]) == 1, childrenAsTrees[1]["children"]
 #                     print("CHILDREN IN THE RC", [x["category"] for x in childrenAsTrees[1]["children"]])
                      firstVP = [x["category"] for x in childrenAsTrees[1]["children"]].index("VP")
#                      print("First VP", firstVP)
                      childrenInTheVP = [x["category"] for x in childrenAsTrees[1]["children"][firstVP]["children"]]
                      print("CHILDREN IN THE VP", childrenInTheVP)
                      if len(childrenInTheVP) > 1 and childrenInTheVP[0].startswith("VB") and childrenInTheVP[1] == "NP":
                    
     #                 print("Embedded verb head", sentence[childHeads[0]-1])
      #                print("Is the last word of RC?", sentence[childHeads[0]-1]["word"] == leaves[-1])
                           totalCountRCs += 1
                           totalCountObjectIsLast += (1 if len(childrenInTheVP) == 2 else 0)
                           print(totalCountObjectIsLast / float(totalCountRCs), totalCountRCs) # about 63%
