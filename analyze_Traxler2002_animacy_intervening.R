...
data = data.frame()
for(model in models) {
   data2 = read.csv(paste("output/Traxler2002_expt4_english_", model, sep=""), sep="\t")
   data2$Model = model
   data = rbind(data, data2)
}
data = data %>% filter(!grepl("OOV", RegionLSTM))
# from ~/scr/recursive-prd/BarteketalJEP2011data/master.tex
 raw.spr.data <- read.csv("../stimuli/traxler_etal_2002/expt4-tokenized.tsv", sep="\t")
...
raw.spr.data = raw.spr.data %>% mutate(long.C=grepl("_long", Condition)-0.5)


#summary(lmer(Surprisal ~ ORC.C + (1+ORC.C|Item) + (1+ORC.C|Model), data=raw.spr.data %>% filter(Region == "v0")))

#Matrix verb
summary(lmer(Surprisal ~ ORC.C*animate.C*long.C + (1+ORC.C+animate.C+long.C|Item) + (1+ORC.C+animate.C+long.C|Model), data=raw.spr.data %>% filter(Region == "v1")))
...
raw.spr.data = raw.spr.data %>% mutate(RegPosition = case_when(Region == "d0" ~ 1, Region == "n0" ~ 2, Region == "c" ~ 3, Region == "d1" & ORC.C > 0 ~ 4, Region == "n1" & ORC.C > 0 ~ 5, Region == "v0" & ORC.C > 0 ~ 6, Region == "v1" ~ 7, Region == "post" ~ 8, Region == "v0" & ORC.C<0 ~ 4, Region == "d1" & ORC.C<0 ~ 5, Region == "n1" & ORC.C<0 ~ 6, Region == "intervening" ~ 6.5))
raw.spr.data = raw.spr.data %>% mutate(RegWord = case_when(Region == "d0" ~ "the", Region == "n0" ~ "banker", Region == "c" ~ "that", Region == "d1" ~ "the", Region == "n1" ~ "lawyer", Region == "v0" ~ "irritated", Region == "v1" ~ "played", Region == "post" ~ "tennis every Sunday", Region == "intervening" ~ "yesterday"))
