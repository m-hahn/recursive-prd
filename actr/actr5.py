...
#      logits = output(out_full) 
      ...
      for i in range(args.sequence_length):
          ...
          print(i, "MEAN SQUARE", out_encoder.pow(2).mean())
          attention_logits = torch.bmm(attention_proj(out_encoder).transpose(0,1), out_decoder.transpose(0,1).transpose(1,2))
          attention = attention_softmax(attention_logits).transpose(0,1)
          if printHere:
             print(attention_logits[0].view(-1))
             print(attention[:,0], attention.size(), i)
          from_encoder = (out_encoder.unsqueeze(2) * attention.unsqueeze(3)).sum(dim=0).transpose(0,1)
