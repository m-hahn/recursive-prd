...
parser.add_argument("--weight_dropout_in", type=float, default=random.choice([0.1, 0.2]))
parser.add_argument("--weight_dropout_out", type=float, default=random.choice([0.01, 0.05, 0.1])) # , 0.2, 0.3
parser.add_argument("--char_dropout_prob", type=float, default=random.choice([0.01, 0.05, 0.1, 0.2])) # , 0.1, 0.2, 0.3
#parser.add_argument("--char_noise_prob", type = float, default=random.choice([0.0]))
parser.add_argument("--learning_rate", type = float, default= random.choice([0.8, 1.0, 1.2])) # 0.1, 0.15, 0.2, 0.3, 0.5, 2.0
...
parser.add_argument("--lr_decay_after_failure", type=float, default=random.choice([0.5, 0.7, 0.8])) # , 1.0, 0.9, 0.95
...
attention_proj = torch.nn.Linear(2*args.hidden_dim, args.hidden_dim).cuda()
...
def forward(numeric, train=True, printHere=False):
      ...
      for i in range(args.sequence_length):
          ...
          if len(outputsSoFar) > 0:
             ...
             attention_logits = attention_out(tanh(attention_proj(torch.cat([out_encoder_keys, out_decoder.expand(i, -1, -1)], dim=2))))
