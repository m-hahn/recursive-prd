...
#             print([x.size() for x in fluctuatingActivations])
             ...
             powerDistances = torch.FloatTensor([(i-j+1) ** (-0.5) for j in range(0, i+1)]).cuda().unsqueeze(0).unsqueeze(0)
             multiplied = torch.log((prior_activations * powerDistances).sum(dim=2) + 1e-8)
