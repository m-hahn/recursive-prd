...
def load(language, partition="train", removeMarkup=True, tokenize=True):
  ...
  with open("/u/scr/mhahn/recursive-prd/VSLK_LCP/E5_EN_SPR/data/rawdata.txt", "r") as inFile:
     text = [x.split(" ") for x in inFile.read().strip().split("\n")]
  header = ["subject","expt","item","condition","position","word","response","rt"] # according to VSLK_LCP/E5_EN_SPR/analysis/rcode$ vim analysisv2.R
  ...
  for linenum, line in enumerate(text):
     ...
     if line[header["position"]] == "?": # Reove yes-no questions
         continue
     words = words.split("_")
     for word in words:
