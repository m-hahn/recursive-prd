...
#          out = dropout(out)
 ...
 #     print(target_tensor)
      ...
      for i in range(0,args.sequence_length): #range(1,maxLength+1): # don't include i==0
         ...
         completeData.append((lineNum, itos[numericCPU[i+1][j]-3], losses[i][j]))
...
if True:
   ...
   test_data = corpusIteratorGT.load(args.language, "test", tokenize=False)
   print("Got data")
...
with open("output/"+"GT"+"_"+args.load_from, "w") as outFile:
   print("\t".join(["LineNumber", "Type", "Item", "Condition", "Iteration", "Word", "Surprisal"]), file=outFile)
   for num, entry in enumerate(completeData):
     print(entry)
     print("\t".join([str(x) for x in [num, entry[0][0], entry[0][1], entry[0][2], entry[0][3], entry[1], entry[2]]]), file=outFile)
