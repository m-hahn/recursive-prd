...
models = sorted([x for x in os.listdir(path) if x.startswith("estimates-"+language+"_") and "words_NoNewWeightDrop_BPE_RunTest.py" in x])

import subprocess

print("results/models_vanillaLSTM_BPE_"+language+".tsv")
with open("results/models_vanillaLSTM_BPE_"+language+".tsv", "w") as outFile:
