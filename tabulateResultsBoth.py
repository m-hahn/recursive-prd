...
if True:
   ...
   for filen in logs:
      ...
      results.append((deletion_rate, round(predictionLoss,4), round(reconstructionLoss, 4), " ".join(params), version, filenum, load_from_lm))
      results2.append((deletion_rate, predictionLoss, reconstructionLoss, " ".join(params), version, filenum, load_from_lm) + tuple([params_here.get(x, "NA") for x in PARAMS]))
   ...
   with open("lm_noise/tableSearchResults.tsv", "w") as outFile:
     print("\t".join(["deletion_rate", "predictionLoss", "prediction_loss", "params", "version", "filenum", "load_from_lm"] + PARAMS), file=outFile)
     for r in results:
