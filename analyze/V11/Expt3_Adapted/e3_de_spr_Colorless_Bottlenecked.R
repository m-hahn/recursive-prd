...
data <- read.csv("../../../../stimuli/V11/colorlessGreen_German.txt", sep="\t")
...
datModel = data.frame()
for(model in models) {
   datModel2 = tryCatch(read.csv(paste("../../../output/V11_E3_ColorlessGreen_german_", model, sep=""), sep="\t") %>% mutate(Model = model), error=function(q) 1)
   if(datModel2 != 1) {
     cat(model,"\n")
