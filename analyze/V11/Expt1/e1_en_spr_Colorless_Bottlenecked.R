...
datModel = data.frame()
for(model in models) {
   datModel2 = tryCatch(read.csv(paste("../../../output/V11_E1_ColorlessGreen_english_", model, sep=""), sep="\t") %>% mutate(Model = model), error=function(q) 1)
   if(datModel2 != 1) {
     cat(model,"\n")
...
mean(grepl("OOV", datModel$RegionLSTM))
...
data$i <- 1 #ifelse(data$Condition%in%c("a","c"),1,-1)
data$gxi <- 1 #ifelse(data$Condition%in%c("a","d"),1,-1)
...
plot = ggplot(data3 %>% group_by(grammatical, ModelPerformance, Model) %>% summarise(Surprisal=mean(Surprisal)), aes(x=grammatical, y=Surprisal, group=Model)) + geom_line() + facet_wrap(~ModelPerformance)
# medium models show forgetting, good models show no difference (expected in the colorless geen setting), and bad models show no difference either (as expected)
...
plot = ggplot(data4 %>% group_by(grammatical, ModelPerformance, Model) %>% summarise(Surprisal=mean(Surprisal)), aes(x=grammatical, y=Surprisal, group=Model)) + geom_line() + facet_wrap(~ModelPerformance) # this plot looks quite clear and convinving: bad models show nothing, medium models show forgetting, good models show correct difference
...
data$gram <- ifelse(data$Condition%in%c("g"),"gram","ungram")
data$int <- 1 #ifelse(data$Condition%in%c("a","c"),"hi","lo")
