...
datModel = data.frame()
for(model in models) {
   datModel2 = read.csv(paste("~/scr/CODE/recursive-prd/output/V11_E5_english_", model, sep=""), sep="\t") %>% mutate(Model = model)
...
summary(modelv1 <- lmer(Surprisal~gram+int+gxi+(1|Model)+(1|item), v1))
...
summary(modelpostv1 <- lmer(Surprisal~gram+int+gxi+(1+gram|Model)+(1|item),
                            postv1))
