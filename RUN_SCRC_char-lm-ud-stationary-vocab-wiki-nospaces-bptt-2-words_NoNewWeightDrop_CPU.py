...
rnn = torch.nn.LSTM(2*args.word_embedding_size, args.hidden_dim, args.layer_num)
...
word_embeddings = torch.nn.Embedding(num_embeddings=len(itos)+3, embedding_dim=args.word_embedding_size)
...
char_composition = torch.nn.LSTM(args.char_emb_dim, args.char_enc_hidden_dim, 1, bidirectional=True)
char_composition_output = torch.nn.Linear(2*args.char_enc_hidden_dim, args.word_embedding_size)

char_decoder_rnn = torch.nn.LSTM(args.char_emb_dim + args.hidden_dim, args.char_dec_hidden_dim, 1)
...
if args.load_from is not None:
  checkpoint = torch.load("/u/scr/mhahn/CODEBOOKS/"+args.language+"_"+__file__.replace("RUN_SCRC_", "").replace("_CPU", "")+"_code_"+str(args.load_from)+".txt", map_location="cpu")
...
def prepareDatasetChunks(data, train=True):
      ...
      for chunk, chunk_line_numbers in data:
       ...
       if len(numerified) > (args.batchSize*args.sequence_length):
         ...
         line_numbersCurrent = torch.LongTensor(line_numbersCurrent).view(args.batchSize, -1, sequenceLengthHere).transpose(0,1).transpose(1,2)
...
zeroBeginning = torch.LongTensor([0 for _ in range(args.batchSize)]).view(1,args.batchSize)
beginning = None
...
if True:
   ...
   test_data = corpusIterator_SCRC.load(args.language, section=args.section, tokenize=True)
   print("Got data")
...
with open("output/SCRC"+args.section+"_"+args.language+"_"+args.load_from, "w") as outFile:
