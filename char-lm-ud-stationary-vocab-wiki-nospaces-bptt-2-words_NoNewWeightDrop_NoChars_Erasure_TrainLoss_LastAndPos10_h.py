...
parser.add_argument("--learning_rate", type = float, default= random.choice([0.000005, 0.00001, 0.00002, 0.00005, 0.0001, 0.0002, 0.0003, 0.0005, 0.001]))
...
parser.add_argument("--bilinear_l2", type=float, default=random.choice([1e-10, 1e-9, 1e-8, 1e-7, 1e-6, 1e-5, 1e-4]))
...
memory_mlp_inner_from_pos = torch.nn.Linear(500, 500).cuda()
memory_mlp_outer = torch.nn.Linear(500, 1).cuda()
memory_mlp_positions_bilinear = torch.nn.Linear(256, 500).cuda()
...
memory_bilinear = torch.nn.Linear(500, 500, bias=False).cuda()
memory_bilinear.weight.data.fill_(0)


##############
modules_memory = [memory_mlp_inner, memory_mlp_outer, memory_mlp_inner_from_pos, positional_embeddings, perword_baseline_inner, perword_baseline_outer, memory_word_pos_inter, memory_bilinear, memory_mlp_inner_bilinear, memory_mlp_positions_bilinear]
...
#      print(numeric_transformed.size(), embedded_everything.size())
   ...
   #   print(embedded_positions.size())
  #    print(relu(memory_mlp_positions_bilinear(embedded_positions)).size())
 #     print(memory_bilinear(relu(memory_mlp_positions_bilinear(embedded_positions))).size())
#      quit()
      attention_bilinear_term = torch.bmm(memory_bilinear(relu(memory_mlp_positions_bilinear(embedded_positions))), relu(memory_mlp_inner_bilinear(embedded_everything.detach())).transpose(1,2)).transpose(1,2)
      #print(
...
#      baselineValues = perword_baseline(target_tensor[-1]).squeeze(1)
      ...
      loss += args.bilinear_l2 * (memory_bilinear.weight.pow(2).sum())
#      quit()
