# char-lm-ud-stationary-vocab-wiki-nospaces-bptt-2-words_NoNewWeightDrop_NoChars_Erasure_TrainLoss_LastAndPos10_k.py
# Based on e (explores high l2 weights -- doesn't work well)
...
parser.add_argument("--bilinear_l2", type=float, default=random.choice([1e-2, 1e-1, 0.5, 1]))
...
#      baselineValues = perword_baseline(target_tensor[-1]).squeeze(1)
      ...
      loss += args.bilinear_l2 * (numeric_embedded.pow(2).mean() + memory_hidden_logit_per_wordtype.pow(2).mean() + attention_bilinear_term.pow(2).mean())
