...
class LanguageModel(torch.nn.Module):
  def __init__(self):
      ...
      self.zeroBeginning = torch.LongTensor([0 for _ in range(args.batchSize)]).view(1,args.batchSize)
      self.beginning = None
...
class Autoencoder(torch.nn.Module):
  def __init__(self):
      ...
      self.zeroHidden = torch.zeros((args.layer_num, args.batchSize, args.hidden_dim_autoencoder))
      
      self.bernoulli = torch.distributions.bernoulli.Bernoulli(torch.tensor([0.1 for _ in range(args.batchSize)]))
      
      self.bernoulli_input = torch.distributions.bernoulli.Bernoulli(torch.tensor([1-args.weight_dropout_in for _ in range(args.batchSize * 2 * args.word_embedding_size)]))
      self.bernoulli_output = torch.distributions.bernoulli.Bernoulli(torch.tensor([1-args.weight_dropout_out for _ in range(args.batchSize * 2 * args.hidden_dim_autoencoder)]))
  ...
  def forward(self, numeric, train=True, printHere=True):
      ...
      numeric_noised = torch.LongTensor([[0 for _ in range(args.sequence_length-len(y))] + y for y in numeric_noised]).t()
...
#          print(probs.size(), probs.sum(dim=2))
 #         quit()
      ...
      return result, torch.LongTensor(result_numeric)
...
if args.load_from_lm is not None:
  checkpoint = torch.load("/u/scr/mhahn/CODEBOOKS/"+args.language+"_"+lmFileName+"_code_"+str(args.load_from_lm)+".txt", map_location="cpu")
...
if args.load_from_autoencoder is not None:
  checkpoint = torch.load("/u/scr/mhahn/CODEBOOKS/"+args.language+"_"+autoencoderFileName+"_code_"+str(args.load_from_autoencoder)+".txt", map_location="cpu")
  for i in range(len(checkpoint["components"])):
