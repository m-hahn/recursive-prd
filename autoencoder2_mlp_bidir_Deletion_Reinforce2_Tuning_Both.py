...
parser.add_argument("--RATE_WEIGHT", type=float, default=random.choice([5.0])) #, 3.5, 4.0, 4.5, 5.0, 5.5, 6.0]))
 # 1.5, 2.0, 2.5,  3.0, 3.5, 
...
#      global runningAveragePredictionLoss
      ...
      if printHere:
         ...
         for i in range((args.sequence_length)):
            print((losses[i][0], itos_total[numericCPU[i+1][0]], memory_hidden_CPU[i], itos_total[numeric_noisedCPU[i+1][0]]))
