...
# char-lm-ud-stationary-vocab-wiki-nospaces-bptt-2-words_NoNewWeightDrop_NoChars_Erasure_Continuous.py
# Preparing for Kumaraswamy version
...
#      numeric_noised = [[x if random.random() > args.deletion_rate else 0 for x in y] for y in numeric.cpu().t()]
 #     numeric_noised = torch.LongTensor([[0 for _ in range(args.sequence_length-len(y))] + y for y in numeric_noised]).cuda().t()
      ...
      uniform_samples = torch.cuda.FloatTensor(args.sequence_length, args.batchSize).uniform_()
      
      l,r=-0.1, 1.1
      stretched_sample = l+(r-l)*uniform_samples
      stretched_sample = torch.clamp(stretched_sample, min=0, max=1)
      embedded = stretched_sample.unsqueeze(2) * embedded + (1-stretched_sample.unsqueeze(2)) * word_embeddings.weight[0].view(1,1,-1)
...
#         boundaries_index = [0 for _ in numeric]
         ...
         stretched_sample_cpu = stretched_sample[:,0].cpu()
 ...
 #           if boundaries_index[0] < len(boundaries[0]) and i+1 == boundaries[0][boundaries_index[0]]:
  #             boundary = True
   #            boundaries_index[0] += 1
    #        else:
     #          boundary = False
            print((losses[i][0], itos_total[numericCPU[i+1][0]], float(stretched_sample_cpu[i])))
