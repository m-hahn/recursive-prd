...
parser.add_argument("--learning_rate", type = float, default= random.choice([1e-7, 1e-6])) #, 0.00002, 0.00005, 0.0001, 0.0002, 0.0003]))
...
parser.add_argument("--dual_learning_rate", type=float, default=random.choice([0.002]))
TRAIN_LM = False
assert not TRAIN_LM
...
parser.add_argument("--momentum", type=float, default=random.choice([0.9]))
...
#      baselineValues = perword_baseline(target_tensor[-1]).squeeze(1)
      ...
      retentionTarget = 1-args.deletion_rate if updatesCount > 1000000 else 1.0
      loss = 0
      # Overall Reward
...
maxUpdates = 10000000 if args.tuning == 1 else 10000000000
